  ...
  return new Promise(function (resolve, reject) {
    wx.login({
      success: function (res) {
        http.post({
          url: 'sxps_seller/user.php?m=login',
          data: { code: res.code }
        }).then(function (res) {
          if (res.errno === 0) {
...
function getUser(options = {}) {
  return new Promise(function (resolve, reject) {
    ...
    if (user && !options.nocache) {
      ...
      http.get({
        url: 'sxps_seller/user.php?m=get',
        data: options
      }).then(function (res) {
        if (res.errno === 0) {
          wx.setStorageSync('user', res.user)
...
function setUser(options) {
  return new Promise(function (resolve, reject) {
    http.post({
      url: 'sxps_seller/user.php?m=set',
      data: options
    }).then(function (res) {
      if (res.errno === 0) {
        let user = wx.getStorageSync('user')
...
function mobileCodeRequest(mobile) {
  return new Promise(function (resolve, reject) {
    http.post({
      url: 'sxps_seller/mobile.php?m=codeRequest',
      data: {
        tplId: 29922,
        mobile: mobile
      }
...
function mobileCodeVerify(mobile, code) {
  return new Promise(function (resolve, reject) {
    http.post({
      url: 'sxps_seller/mobile.php?m=codeVerify',
      data: { mobile, code },
    }).then(function (res) {
      resolve(res)
